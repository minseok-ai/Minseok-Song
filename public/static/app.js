...
function domainLogoSrc(key) {
  return key === "nnfc" ? "/static/NNFC.png" : "/static/A1Firm.png";
}
...
const DOMAIN_PROMPTS = {
  nnfc: `만들고 싶은 소자·박막·공정을 편하게 적어 주세요. (논문·수치가 없어도 됩니다)

e.g., '실리콘 웨이퍼 위에 약 200nm SiO2 캡을 씌우고 싶습니다. NNFC 장비로 가능한 대략적인 공정 순서와 추천 장비 초안을 알려주세요.'`,
  ...
  nnfc: [
    {
      ...
      prompt: "실리콘 웨이퍼 위에 약 200nm SiO2 절연막을 형성하려고 합니다. NNFC 보유 장비 기준으로 가능한 공정 순서, 후보 장비, 주요 파라미터, 엔지니어 확인이 필요한 hold point를 초안으로 정리해주세요."
    },
    {
      label: "Al 전극 패터닝",
    ...
    {
      label: "샘플 조건 기반 검토",
      prompt: "샘플 기판, 목표 박막 두께, 열 예산, 사용 가능 가스 조건을 입력하면 NNFC 장비 매칭과 공정 리스크를 함께 검토하는 runsheet 초안을 만들어주세요."
    }
  ],
  business: [
...
function inferRunSheetHoldPoint(card, matches, note) {
  ...
  if (/deposition|PECVD|TEOS|gas|plasma|NF3|O2|temperature|temp|°C|℃/i.test(combined)) return "Recipe setpoint approval";
...
function buildNnfcRunSheetRows(processCards, reportText, equipLog, dbMatches = []) {
  ...
  const rows = sourceCards.slice(0, 12).map((card, index) => {
    ...
    return {
      ...
      recipe: note || "Set baseline parameters after engineer review.",
...
function renderNnfcRunSheet(processCards, reportText, equipLog, dbMatches = []) {
  ...
  if (!rows.length) {
    return `<p class="empty-state">Run sheet draft is not available. Check the Engineer Detail tab for the validation log.</p>`;
  }
  ...
  return `
    <div class="runsheet-panel">
      <div class="runsheet-header">
        <div>
          <div class="results-kicker">NNFC operations draft</div>
          <h3>Run Sheet Draft</h3>
        </div>
        ...
        <table class="runsheet-table">
          <thead>
            <tr>
              ...
              <th>Baseline Recipe / Parameter</th>
              <th>Hold Point</th>
              <th>Owner</th>
            </tr>
          </thead>
...
window.downloadRunSheetCSV = function() {
  ...
  const header = ["No", "Process Step", "Equipment / Tool", "Baseline Recipe / Parameter", "Hold Point", "Owner"];
...
function renderNnfcNextActions(processCards, equipLog, dbMatches) {
  ...
  return `
    <div class="nnfc-next-actions" aria-label="다음 확인 항목">
      ...
      <div class="nnfc-next-action">
        <span class="nnfc-next-label">Run Sheet</span>
        ...
        <div class="results-card results-card-nnfc">
          ...
          <div class="results-header">
            <div>
              ...
              <h2>Baseline Recipe Draft</h2>
              <p class="results-subtitle">고객용 요약 · 엔지니어 상세는 별도 탭에서 확인</p>
            </div>
            ${resultActionsHtml}
          </div>
          ...
          <div class="view-tabs view-tabs-nnfc secondary-view-dock">
            <button id="btn-runsheet" class="view-tab active" type="button" onclick="window.setViewMode('runsheet')">Run Sheet</button>
